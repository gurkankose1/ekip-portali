...
const NavLink: React.FC<{
    ...
    <button
        ...
        title={title}
        aria-label={title}
        className={`flex items-center gap-2 px-3 py-2 text-sm font-semibold rounded-md transition-colors duration-200 ${isActive ? 'bg-sky-600 text-white' : 'text-slate-300 hover:bg-slate-700'
            }`}
    >
        {icon}
...
export const AppHeader: React.FC<AppHeaderProps> = ({ activeView, setActiveView, theme, setTheme, currentUser, onLogout, isMusicPlayerVisible, onToggleMusicPlayer }) => {
    ...
    return (
        <header className="w-full p-2 z-30 bg-slate-900/90 backdrop-blur-sm border-b border-slate-700">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    <div className="flex items-center gap-1 p-1 rounded-lg bg-slate-700">
                        ...
                        <button
                            ...
                            className={`p-1.5 rounded-md transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-sky-500 ${isMusicPlayerVisible
