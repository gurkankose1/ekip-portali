...
const DayCard: React.FC<{ 
    ...
    onRemoveReinforcement: (date: Date, personnel: string) => void;
    leaves: Map<string, Set<string>>;
}> = ({ day, userRole, onToggleLeave, onAddReinforcement, onRemoveReinforcement, leaves }) => {
                    ...
                    {assignments.map(a => {
                        ...
                        return (
                            <li key={a.personnel} className="flex justify-between items-center text-sm gap-2">
                               ...
                               {a.shift === 'Yıllık İzin' ? (
                                   <span className="font-mono text-xs bg-emerald-900/80 text-emerald-300 px-2 py-0.5 rounded-md">YILLIK İZİN</span>
                               ) : a.station ? (
                               ...
                               {userRole === 'admin' && (canTakeLeave || isOnLeave) && (
                                    <button
                                        ...
                                        className={`text-xs font-semibold px-2 py-0.5 rounded-md transition-colors duration-200 ${
                                            isOnLeave 
                                    ...
                                    >
                                        {isOnLeave ? 'İzni İptal Et' : 'İzin Ver'}
                                    </button>
                               )}
                            </li>
                        );
...
export const ScheduleView: React.FC<ScheduleViewProps> = ({ schedule, userRole, onToggleLeave, onAddReinforcement, onRemoveReinforcement, leaves }) => {
